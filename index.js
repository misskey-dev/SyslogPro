 ...
 * @since 0.0.0
 */
class Syslog extends events_1.default {
    /**
    ...
    addTlsServerCerts(certs) {
        if (Array.isArray(certs)) {
            ...
            let errMsg = 'TYPE ERROR: Server Cert should be a Buffer/string';
...
class RFC extends Format {
    ...
    setColor(colors, extendedColor) {
        if (colors.emergencyColor) {
            try {
                this.emergencyColor = rgbToAnsi(colors.emergencyColor, this.extendedColor);
            }
            catch (reason) {
        ...
        if (colors.criticalColor) {
            try {
                this.criticalColor = rgbToAnsi(colors.criticalColor, this.extendedColor);
            }
            catch (reason) {
                reason.message = 'TYPE ERROR: ';
        ...
        if (colors.informationalColor) {
            try {
                this.informationalColor = rgbToAnsi(colors.informationalColor, this.extendedColor);
            }
            catch (reason) {
                reason.message = 'TYPE ERROR: ';
...
class RFC3164 extends RFC {
    ...
    buildMessage(msg, options) {
        ...
        const timestamp = moment_1.default(options.timestamp)
    ...
    buildMessage(msg, options) {
        ...
        if (this.timestamp || options.timestamp) {
            if (this.timestampUTC) {
                if (this.timestampMS) {
                    if (this.timestampTZ) {
                        timestamp = moment_1.default(options.timestamp).toISOString();
                    }
                    else {
                        timestamp = moment_1.default(options.timestamp)
                            .utc()
                ...
                if (this.timestampTZ) {
                    ...
                    if (this.timestampMS) {
                        ...
                        timestamp = moment_1.default(options.timestamp).format('YYYY-MM-DDThh:mm:ss');
                    }
                }
            }
        }
